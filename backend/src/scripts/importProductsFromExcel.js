...
function calculateExpectedProfit(row) {
  ...
  if (purchasePrice === null || shippingCost === null || salePrice === null)
    return null;
...
function mapProducts(headers, dataRows) {
  ...
  for (const { rowNumber, row } of dataRows) {
    ...
    const defaultPurchasePrice = normalizeMoney(
      readValue(record, purchasePriceHeaders),
    );
    ...
    const reportedProfit =
      record["Lãi"] == null || String(record["Lãi"]).trim() === ""
        ? null
    ...
    if (defaultPurchasePrice === null)
      errors.push({ rowNumber, field: "Giá nhập", message: "Phải là số >= 0" });
    ...
    if (defaultSalePrice === null)
      errors.push({ rowNumber, field: "Giá bán", message: "Phải là số >= 0" });
    ...
    if (name) {
      ...
      if (previousRow) {
        warnings.push({
          rowNumber,
          field: "Tên sản phẩm",
          message: `Trùng với dòng ${previousRow}`,
        });
      } else {
  ...
  return {
    ...
    warnings,
    products,
    categoryNames: [...categoryNames].sort((a, b) => a.localeCompare(b, "vi")),
  };
}
...
function printPlan({
  ...
  console.log(
    "- Compare Lai with calculated sale - purchase - shipping and warn on mismatch",
  );
...
async function main() {
  ...
  const { errors, warnings, products, categoryNames } = mapProducts(
    headers,
    dataRows,
  );
  ...
  if (!write) {
    console.log("");
    console.log(
      "Dry-run only. Re-run with --write to apply the import after review.",
    );
    return;
  }
  ...
  console.log("");
  console.log(
    `Imported ${products.length} products and ${categoryNames.length} categories.`,
  );
}
