...
function basicOrderErrors(input) {
  ...
  if (payload.deliveryFee !== undefined) {
    ...
    if (!Number.isFinite(deliveryFee) || deliveryFee < 0) {
      errors.push({
        field: "deliveryFee",
        message: "Phí giao hàng phải là số lớn hơn hoặc bằng 0",
      });
    }
  }
  return errors;
...
export class OrderService {
  ...
  async list({ search = "", status = "", fromDate = "", toDate = "" } = {}) {
    if (status && !isValidOrderStatus(status)) {
      throw validationError([
        { field: "status", message: "Trạng thái đơn hàng không hợp lệ" },
      ]);
    }
  ...
  async create(payload) {
    payload = payload && typeof payload === "object" ? payload : {};
    const order = await this.normalize({
      ...payload,
      status: payload.status || "PENDING_PURCHASE",
    });
  ...
  async dashboardSummary(totalProducts) {
    ...
    return {
      ...
      pendingOrders: orders.filter((order) => order.status !== "COMPLETED")
        .length,
      ...
      totalProducts,
      ordersByStatus,
      monthlyRevenue: monthly.map(({ month, totalRevenue }) => ({
        month,
        totalRevenue,
      })),
      monthlyProfit: monthly.map(({ month, totalProfit }) => ({
        month,
        totalProfit,
      })),
