...
function validateProduct(input) {
  ...
  if (!String(payload.productCategoryName || '').trim()) {
    errors.push({ field: 'productCategoryName', message: 'Phân loại mặt hàng không được để trống' })
  }
...
export class ProductService {
  ...
  async list({ search = '', purchaseLocation = '', productCategoryName = '' } = {}) {
    ...
    return products
      ...
      .filter(product => matchesSearchText(product.purchaseLocation, purchaseLocation))
      .filter(product => !normalizedCategory || normalizeSearchText(product.productCategoryName).includes(normalizedCategory))
  ...
  async resolveCategory(payload, current = {}) {
    const categoryName = String(payload.productCategoryName || current.productCategoryName || '').trim()
    ...
    const products = await this.productRepository.findAll()
    const existing = products.find(product => normalizeSearchText(product.productCategoryName) === normalizeSearchText(categoryName))
    return {
      id: existing?.productCategoryId || current.productCategoryId || `CAT-${randomUUID().slice(0, 8).toUpperCase()}`,
  ...
  async create(payload) {
    ...
    return this.productRepository.create(normalizeProduct(payload, {}, category))
  }
  ...
  async update(id, payload) {
    ...
    return this.productRepository.update(id, normalizeProduct(payload, current, category))
  }
