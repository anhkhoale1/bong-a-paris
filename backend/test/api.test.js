...
describe("Sales Manager API", () => {
  ...
  const authenticate = (requestBuilder) =>
  ...
  it("từ chối phí giao hàng âm", async () => {
    ...
    expect(response.body.errors).toContainEqual(
      expect.objectContaining({ field: "deliveryFee" }),
    );
  });
  ...
  it("cho phép tạo đơn khi không có nơi nhập thực tế", async () => {
    const response = await authenticate(
      request(server)
        ...
        .send(
          validOrder({
            items: [{ ...validOrder().items[0], purchaseLocation: "" }],
          }),
        ),
    ).expect(201);
  ...
  it("cho phép tạo đơn khi không có địa chỉ giao hàng", async () => {
    const response = await authenticate(
      request(server)
        .post("/api/orders")
        .send(validOrder({ customerAddress: "" })),
    ).expect(201);

    expect(response.body.data.customerAddress).toBe("");
  });
  ...
  it("từ chối số lượng không phải số nguyên dương", async () => {
    const response = await authenticate(
      request(server)
        .post("/api/orders")
        .send(
          validOrder({ items: [{ ...validOrder().items[0], quantity: 0 }] }),
        ),
    ).expect(400);
  ...
  it("lưu completedAt khi hoàn thành và xóa khi chuyển về trạng thái khác", async () => {
    const created = await authenticate(
      request(server).post("/api/orders").send(validOrder()),
    ).expect(201);
    ...
    const reopened = await authenticate(
      request(server)
        .patch(`/api/orders/${id}/status`)
        .send({ status: "SHIPPED" }),
    ).expect(200);
  ...
  it("tạo và cập nhật sản phẩm", async () => {
    ...
    const created = await authenticate(
      request(server).post("/api/products").send(payload),
    ).expect(201);
    expect(created.body.data.name).toBe(payload.name);
    expect(created.body.data.productCategoryName).toBe(
    ...
    expect(
      new Date(updated.body.data.updatedAt).getTime(),
    ).toBeGreaterThanOrEqual(new Date(created.body.data.updatedAt).getTime());
  });

  it("liệt kê và lọc sản phẩm theo phân loại", async () => {
    await authenticate(
      request(server).post("/api/products").send({
    ...
    const categories = await authenticate(
      request(server).get("/api/products/categories"),
    ).expect(200);
    ...
    const filtered = await authenticate(
      request(server).get("/api/products?productCategoryName=Kem chống nắng"),
    ).expect(200);
  ...
  it("tìm sản phẩm không dấu và viết tắt", async () => {
    ...
    const noAccent = await authenticate(
      request(server).get("/api/products?search=xanh la"),
    ).expect(200);
    expect(noAccent.body.data).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: "KCN La Roche-Posay xanh lá" }),
      ]),
    );

    const abbreviation = await authenticate(
      request(server).get("/api/products?search=lrp xanh"),
    ).expect(200);
    expect(abbreviation.body.data).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: "KCN La Roche-Posay xanh lá" }),
      ]),
    );
  ...
  it("đọc lại dữ liệu đã ghi khi khởi tạo ứng dụng mới", async () => {
    ...
    try {
      const response = await authenticate(
        request(restartedServer).get(`/api/products/${created.body.data.id}`),
      ).expect(200);
